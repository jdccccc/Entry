...
use ratatui::{
    ...
    text::{Line, Span},
    // 常用小部件：边框块、列表、段落等
    widgets::{Block, Borders, List, ListItem, Paragraph, ListState},  // 添加 ListState
    // 终端抽象
    Terminal,
};
...
impl MenuItem {
    ...
    // 返回所有菜单项的固定数组（长度为 2）
    fn all() -> [MenuItem; 2] { [MenuItem::Todo, MenuItem::Bill] }

    // 带有 &self 参数，因此这是方法，可用方法调用语法糖：item.title()（等价 MenuItem::title(&item)）。
    // 返回类型：&'static str
    // &str 是字符串切片（只读视图，不拥有数据）
    // 'static 生命周期表示活得和整个程序一样久。字符串字面量（如 "TODO"）就是 &'static str，因为它们被编译进只读数据段，程序运行期间一直存在。
    // match self { ... } 模式匹配：
    // 必须穷尽所有变体，否则编译不过。
